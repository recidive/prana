...
Model.emitAndCall = function(application, type, operation, error, data, callback) {
  ...
  // First invoke the hook with the same name as the operation on all
  // extensions.
  application.invoke(operation, type, data, function(result) {
    if (result) {
      // Process and add items returned from the hook.
      for (var itemName in result) {
        data[itemName] = type.process(itemName, result[itemName]);
      }
    }
...
Model.compile = function(application, type) {
  ...
  MockModel.list = function(query, callback) {
    type.storage.list(query, function(error, items) {
      application.collect(type, items, function(err, result) {
        if (result) {
          for (var itemName in result) {
            items[itemName] = type.process(itemName, result[itemName]);
          }
        }
